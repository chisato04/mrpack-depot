...
foreach ($list_items as $item) {
    ...
    if ($link) {
        ...
        // Get the text that is NOT the link (version, author)
        $mod_info = trim(str_replace($mod_name, '', $item->nodeValue));
    } else {
        // Handle items without a link
        $mod_name = $item->nodeValue;
    }

    // Skip empty/header items
    ...
    <div class="container">
        <div class="details-container">
            ...
            <div class="mod-list-card">
                <h2>Mod List (<?php echo count($mod_list); ?>)</h2>
                        ...
                        <?php foreach ($mod_list as $mod): ?>
                            <tr>
                                <td><a href="<?php echo htmlspecialchars($mod['url']); ?>" target="_blank"
                                        rel="noopener noreferrer"><?php echo htmlspecialchars($mod['name']); ?></a></td>
