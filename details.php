...
// --- VALIDATION ---
if (!isset($_GET['pack'])) { die("No modpack specified."); }
$pack_filename = basename($_GET['pack']);

// --- DATA LOADING ---
$metadata = file_exists($metadataFile) ? json_decode(file_get_contents($metadataFile), true) : [];
if (!isset($metadata[$pack_filename])) { die("Modpack not found in metadata."); }
$pack_meta = $metadata[$pack_filename];
$modlist_html_file = $modlistsDir . '/' . $pack_meta['modlist_file'];
if (empty($pack_meta['modlist_file']) || !file_exists($modlist_html_file)) { die("Modlist file not found."); }

// --- HTML PARSING (Full List) ---
$mod_list = []; // We now use one simple array again
$doc = new DOMDocument();
@$doc->loadHTMLFile($modlist_html_file);
$xpath = new DOMXPath($doc);
$list_items = $xpath->query('//li');
foreach ($list_items as $item) {
    $mod_name = ''; $mod_url = '#'; $mod_info = '';
    ...
    } else { $mod_name = $item->nodeValue; }
    if (empty(trim($mod_name)) || $mod_name === '.connector') continue;
    ...
    <div class="container">
        <div class="details-container">
            <header class="details-header">
                ...
                <a href="<?php echo htmlspecialchars($modpacksDir . '/' . $pack_filename); ?>" class="download-btn" download>Download Pack</a>
            </header>
            ...
            <div class="mod-list-card">
                ...
                <div class="table-wrapper">
                    <table class="mod-list-table">
                        <thead>
                            <tr><th>Mod Name</th><th>Details</th></tr>
                        </thead>
                        <tbody>
                            <?php foreach ($mod_list as $mod): ?>
                                <tr>
                                    <td><a href="<?php echo htmlspecialchars($mod['url']); ?>" target="_blank" rel="noopener noreferrer"><?php echo htmlspecialchars($mod['name']); ?></a></td>
